...
function ScatterModal({ visible, closeModal }) {
  ...
  return (
    <Modal visible={visible} onBackgroundPress={closeModal}>
      <Head status="error">
		{currentLangData.scatterNotFound}
		<div className={`${styles.closeIcon}`} onClick={closeModal}><img src="/img/closeWombat.svg" /></div>
	  </Head>
      <Body>
        <div className={`${styles.desc} ${styles.mt0}`}>
          {currentLangData.pleaseScatter}
		  <br />
		  {currentLangData.walletDownloadStarts}
		  <br />
		  {currentLangData.ifNotPart1} <a className={styles.unders} href={downloadScatter} target="_blank">{currentLangData.ifNotPart2}</a> {currentLangData.ifNotPart3} <a className={styles.unders} href={scatterHome} target="_blank">{currentLangData.ifNotPart4}</a>
        </div>
