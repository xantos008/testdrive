...
function WombatLoginRestartModal() {
  const { currentLangData } = useContext(LangContext)
  const { modal } = useStores()
  
  const closeModal = () => {
    modal.close('wombatLoginRestartModal')
  }
  
  return (
    <Modal visible={modal.visible.wombatLoginRestartModal} onBackgroundPress={closeModal}>
      <Head status="error">
...
export default WombatLoginRestartModal
