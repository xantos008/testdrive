...
function RejectAcceptModal() {
  const { currentLangData } = useContext(LangContext)
  const { modal } = useStores()
  const isMobile = window.innerWidth < 768

  const closeModal = () => {
    modal.close('rejectAccept')
  }

  return (
    <Modal visible={modal.visible.rejectAccept} onBackgroundPress={closeModal} isMobile={isMobile}>
      <Head status="error">
		{currentLangData.yourRequestDenied}
...
export default observer(RejectAcceptModal)
