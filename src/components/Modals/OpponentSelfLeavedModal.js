...
function OpponentSelfLeavedModal() {
  const { currentLangData } = useContext(LangContext)
  const { modal } = useStores()
  const isMobile = window.innerWidth < 768

  const closeModal = () => {
    modal.close('opponentSelfLeaved')
  }

  return (
    <Modal visible={modal.visible.opponentSelfLeaved} onBackgroundPress={closeModal} isMobile={isMobile}>
      <Head status="error">
		{currentLangData.opponentSelfLeaved}
...
export default observer(OpponentSelfLeavedModal)
